...
const editHorary = (horary) => {
    horaryToEdit.value = horary;
    showModal.value = true;
};

const addHorary = () => {
    horaryToEdit.value = null;
    showModal.value = true;
};
...
const changeStatus = (horary) => {
    Swal.fire({
        title: "¿Estás seguro?",
        text: "¿Quieres cambiar el estado de del Horario?",
        icon: "warning",
        ...
        if (result.isConfirmed) {
            form.put(route("horary.change", horary), {
                preserveScroll: true,
            });
        }
    });
...
const goTohoraryIndex = () => {
    window.location.href = route("horary.index");
};
                        ...
                        <div class="flex justify-between py-2 px-4 mr-4 mt-4">
                            <div class="relative">
                                <input type="text" v-model="query"
                                    ...
                                    placeholder="Buscar horario" @keyup.enter="search" @input="query = query.toUpperCase()" />
                                <button @click.prevent="goTohoraryIndex"
                            ...
                            <div>
                                <button class="bg-sky-500 hover:bg-sky-600 p-2 text-white rounded-lg flex items-center"
                                    @click="addHorary">
                            ...
                            <div class="overflow-x-auto rounded-lg">
                                <table class="min-w-full divide-y divide-gray-200">
                                    ...
                                    <tbody class="bg-white divide-y divide-gray-200">
                                        <tr v-for="(horary, index) in horaries.data" :key="horary.id">
                                            ...
                                            <td class="text-xs md:text-sm px-6 py-4 whitespace-nowrap">
                                                <p class="inline-block px-2 rounded-full h-auto justify-center items-center text-sm"
                                                    :class="{
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                ...
                                                <button v-if="$page.props.user.permissions.includes(
                                                    ...
                                                    " class="text-white p-1 rounded-md" :class="{
            'bg-red-400 hover:bg-red-500':
                horary.status == 1,
            'bg-green-500 hover:bg-green-600':
                horary.status == 0,
        }" @click="changeStatus(horary)">
                                                    <v-icon v-if="horary.status == 1" name="gi-cancel" />
                                ...
                                <Pagination :pagination="horaries" />
                            </div>
                        </div>
                    </div>
                </div>
                <Modal :show="showModal" @close="showModal = false">
                    <HoraryForm :horary="horaryToEdit" @close-modal="closeModal" />
                </Modal>
            </div>
        </AppLayout>
    </div>
